...
function configSigmaElements(config) {
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> Typ ' + (x++) + ' (' + sigInst.clusters[b].length + ' Akteure)</a></div>');
    //a.sort();
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (0 >= a.length) this.results.html("<i>Bitte mindestens eine Zahl eingeben!</i>");
        else {
